...
CQuantizer::CQuantizer(UInt32 nMaxColors, UInt32 nColorBits)
{
    ...
    m_pTree = NULL;
    m_nLeafCount = 0;
    for (Int32 i = 0; i <= (Int32)m_nColorBits; i++)
...
BOOL CQuantizer::ProcessImage(HANDLE hImage)
{
    ...
    Int32 effwdt = ((((ds.biBitCount * ds.biWidth) + 31) / 32) * 4);

    Int32 nPad = effwdt - (((ds.biWidth * ds.biBitCount) + 7) / 8);
...
/////////////////////////////////////////////////////////////////////////////
void CQuantizer::AddColor(LPNODE* ppNode, BYTE r, BYTE g, BYTE b, UInt32 nColorBits, UInt32 nLevel, LPDWORD pLeafCount, LPNODE* pReducibleNodes)
{
    static BYTE	mask[8] = { 0x80, 0x40,	0x20, 0x10,	0x08, 0x04,	0x02, 0x01 };

    // If the node doesn't exist, create it.
    if (*ppNode == NULL)
        *ppNode = (LPNODE)CreateNode(nLevel, nColorBits, pLeafCount, pReducibleNodes);
    ...
    {	// Recurse a level deeper if the node is not a leaf.
        Int32	shift = 7 - nLevel;
        Int32	nIndex = (((r & mask[nLevel]) >> shift) << 2) |
...
/////////////////////////////////////////////////////////////////////////////
void* CQuantizer::CreateNode(UInt32 nLevel, UInt32 nColorBits, LPDWORD pLeafCount, LPNODE* pReducibleNodes)
{
    LPNODE pNode = (LPNODE)calloc(1, sizeof(NODE));
...
/////////////////////////////////////////////////////////////////////////////
void CQuantizer::ReduceTree(UInt32 nColorBits, LPDWORD pLeafCount, LPNODE* pReducibleNodes)
{
    // Find	the	deepest	level containing at	least one reducible	node.
    Int32 i = nColorBits - 1;
    for (; (i > 0) && (pReducibleNodes[i] == NULL); i--);
    // Reduce the node most	recently added to the list at level	i.
    LPNODE pNode = pReducibleNodes[i];
    pReducibleNodes[i] = pNode->pNext;

    LPNODE pNodeTmp = pNode;
    LPNODE pNeedReduceNode = NULL, pPreNode = NULL, pPreNodeTmp = NULL;
...
/////////////////////////////////////////////////////////////////////////////
void CQuantizer::DeleteTree(LPNODE*	ppNode)
{
    for (Int32 i = 0; i < 8; i++) {
...
BOOL CQuantizer::FindColorIndex(LPNODE pNode, BYTE r, BYTE g, BYTE b, Int32 nLevel, LPDWORD pColorIndex)
{
    static BYTE	mask[8] = { 0x80, 0x40,	0x20, 0x10,	0x08, 0x04,	0x02, 0x01 };
...
BOOL CQuantizer::GetColorIndex(BYTE r, BYTE g, BYTE b, LPDWORD pColorIndex)
{
    return FindColorIndex(m_pTree, r, g, b, 0, pColorIndex);
}

/////////////////////////////////////////////////////////////////////////////
void CQuantizer::GetPaletteColors(LPNODE pTree, RGBQUAD* prgb, LPDWORD pIndex)
{
    if (pTree) {
        if (pTree->bIsLeaf) {
...
/////////////////////////////////////////////////////////////////////////////
BYTE CQuantizer::GetPixelIndex(LONG x, LONG y, Int32 nbit, LONG effwdt, BYTE *pimage)
{
    if (nbit == 8)
    {
...
CRGBQuantizer::CRGBQuantizer(UInt32 nMaxColors, UInt32 nColorBits)
    :CQuantizer(nMaxColors, nColorBits)
{
}

BOOL CRGBQuantizer::ProcessImageRGB(BYTE *pRGBData, UInt32 nWidth, UInt32 nHeight)
{
    BYTE r, g, b;
...
CBitmapQuantizer::CBitmapQuantizer(UInt32 nMaxColors, UInt32 nColorBits)
    :CQuantizer(nMaxColors, nColorBits)
{
}
...
BOOL CBitmapQuantizer::ProcessImageBitmap(HBITMAP hBmp)
{
    ...
    Int32 nPaletteSize = 0;

    if (bm.bmBitsPixel < 16)
        nPaletteSize = (Int32)pow(2, bm.bmBitsPixel);
    ...
    if (!::GetDIBits(hDC, hBmp, 0, (UInt32)bm.bmHeight, buf, bmpInf, DIB_RGB_COLORS))
    {
        ::ReleaseDC(NULL, hDC);
        LocalFree(bmpInf);
        return FALSE;
