...
hgeParticleSystem::hgeParticleSystem(HGE* hge, const char *filename, hgeSprite *sprite)
{
    void *psi;
...
hgeParticleSystem::hgeParticleSystem(HGE* hge, hgeParticleSystemInfo *psi)
{
    //hge = hgeCreate(HGE_VERSION);
    m_hge = hge;
...
hgeParticleSystem::hgeParticleSystem(HGE* hge, const hgeParticleSystem &ps)
{
...
void hgeParticleSystem::Update(float fDeltaTime)
{
    ...
    if (fAge != -2.0f)
    {
        ...
        for (i = 0; i < nParticlesCreated; i++)
        {
            ...
            par->fAge = 0.0f;
            par->fTerminalAge = m_hge->Random_Float(info.fParticleLifeMin, info.fParticleLifeMax);

            par->vecLocation = vecPrevLocation + (vecLocation - vecPrevLocation)*m_hge->Random_Float(0.0f, 1.0f);
            par->vecLocation.x += m_hge->Random_Float(-2.0f, 2.0f);
            par->vecLocation.y += m_hge->Random_Float(-2.0f, 2.0f);

            ang = info.fDirection - M_PI_2 + m_hge->Random_Float(0, info.fSpread) - info.fSpread / 2.0f;
            if (info.bRelative) ang += (vecPrevLocation - vecLocation).Angle() + M_PI_2;
            par->vecVelocity.x = cosf(ang);
            par->vecVelocity.y = sinf(ang);
            par->vecVelocity *= m_hge->Random_Float(info.fSpeedMin, info.fSpeedMax);

            par->fGravity = m_hge->Random_Float(info.fGravityMin, info.fGravityMax);
            par->fRadialAccel = m_hge->Random_Float(info.fRadialAccelMin, info.fRadialAccelMax);
            par->fTangentialAccel = m_hge->Random_Float(info.fTangentialAccelMin, info.fTangentialAccelMax);

            par->fSize = m_hge->Random_Float(info.fSizeStart, info.fSizeStart + (info.fSizeEnd - info.fSizeStart)*info.fSizeVar);
            par->fSizeDelta = (info.fSizeEnd - par->fSize) / par->fTerminalAge;

            par->fSpin = m_hge->Random_Float(info.fSpinStart, info.fSpinStart + (info.fSpinEnd - info.fSpinStart)*info.fSpinVar);
            par->fSpinDelta = (info.fSpinEnd - par->fSpin) / par->fTerminalAge;

            par->colColor.r = m_hge->Random_Float(info.colColorStart.r, info.colColorStart.r + (info.colColorEnd.r - info.colColorStart.r)*info.fColorVar);
            par->colColor.g = m_hge->Random_Float(info.colColorStart.g, info.colColorStart.g + (info.colColorEnd.g - info.colColorStart.g)*info.fColorVar);
            par->colColor.b = m_hge->Random_Float(info.colColorStart.b, info.colColorStart.b + (info.colColorEnd.b - info.colColorStart.b)*info.fColorVar);
            par->colColor.a = m_hge->Random_Float(info.colColorStart.a, info.colColorStart.a + (info.colColorEnd.a - info.colColorStart.a)*info.fAlphaVar);
