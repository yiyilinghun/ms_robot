...
hgeStringTable::hgeStringTable(HGE* hge, const char *filename)
{
    m_hge = hge;
    int i;
    void *data;
    ...
    data = m_hge->Resource_Load(filename, &size);
    if (!data) return;
    ...
    {
        m_hge->System_Log(STRFORMATERROR, filename);
        delete[] desc;
        return;
    }
    ...
    for (;;)
    {
        ...
        // skip whitespaces to '='
        while (isspace(*pdesc)) pdesc++;
        if (*pdesc != '=') { m_hge->System_Log(STRFORMATERROR, filename); break; }
        pdesc++;
