...
int hgeGUI::Render(__int64 xNow)
{
    hgeGUIObject *ctrl = ctrls;
    ...
    if (m_hge->Input_IsMouseOver() && sprCursor) sprCursor->RenderTexture(mx, my, 0xFFFFFFFF);
...
int hgeGUI::Update(float dt)
{
    ...
    m_hge->Input_GetMousePos(&mx, &my);
    bLPressed = m_hge->Input_KeyDown(HGEK_LBUTTON);
    bLReleased = m_hge->Input_KeyUp(HGEK_LBUTTON);
    bRPressed = m_hge->Input_KeyDown(HGEK_RBUTTON);
    bRReleased = m_hge->Input_KeyUp(HGEK_RBUTTON);
    nWheel = m_hge->Input_GetMouseWheel();
    ...
    {
        if (ctrlFocus->KeyClick(key, m_hge->Input_GetChar())) return ctrlFocus->id;
    }

    // Handle mouse

    bool bLDown = m_hge->Input_GetKeyState(HGEK_LBUTTON);
    bool bRDown = m_hge->Input_GetKeyState(HGEK_RBUTTON);
