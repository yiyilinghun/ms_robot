...
hgeDistortionMesh::hgeDistortionMesh(HGE* hge, int cols, int rows)
{
    int i;
    m_hge = hge;
...
hgeDistortionMesh::hgeDistortionMesh(HGE* hge, const hgeDistortionMesh &dm)
{
    //hge = hgeCreate(HGE_VERSION);
    m_hge = hge;
...
void hgeDistortionMesh::SetTextureRect(float x, float y, float w, float h)
{
    ...
    if (quad.tex)
    {
        tw = (float)m_hge->Texture_GetWidth(quad.tex);
        th = (float)m_hge->Texture_GetHeight(quad.tex);
    }
    else
    {
        tw = w;
